    ...
    <div className="min-h-screen bg-[#050608] text-white flex pt-20">
      ...
      <div className="flex-1 ml-64 p-10 max-w-5xl">
        ...
        {activeTab === 'orders' && (
          <div className="animate-fadeIn">
            <div className="flex justify-between items-end mb-8 border-b border-gray-800 pb-4">
              <h1 className="text-3xl font-bold uppercase tracking-widest">Order History</h1>
              <span className="text-gray-400 text-xs font-bold uppercase tracking-widest bg-gray-900 px-3 py-1 rounded">Total Orders: {ordersList.length}</span>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm text-gray-400">
                  ...
                  <tbody>
                    {ordersList.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="px-6 py-8 text-center text-gray-500 font-mono">No orders found in the database.</td>
                      </tr>
                    ) : (
                      ordersList.map((order) => (
                        <React.Fragment key={order.id}>
                          <tr className="border-b border-gray-800/50 hover:bg-gray-900/30 transition-colors">
                            ...
                            <td className="px-6 py-4 text-white">{order.customer_email || 'Guest User'}</td>
                            <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4 font-bold text-ion">${parseFloat(order.total_amount).toFixed(2)}</td>
                            <td className="px-6 py-4">
                              <select 
                                value={order.status || 'Pending'} 
                                onChange={(e) => handleOrderStatusUpdate(order.id, e.target.value)}
                                className={`text-xs font-bold uppercase tracking-widest rounded px-3 py-1.5 focus:outline-none cursor-pointer border ${
                            ...
                            <td className="px-6 py-4 text-right">
                              <button 
                                ...
                                className="text-[10px] font-bold uppercase tracking-widest bg-gray-800 hover:bg-white hover:text-black px-4 py-2 rounded transition-colors"
                              >
                              ...
                              <td colSpan="6" className="px-8 py-8">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                                  <div>
                                    ...
                                    <p className="text-sm text-white mb-1"><span className="text-gray-500 font-mono text-xs mr-2">NAME:</span> {order.customer_name || 'N/A'}</p>
                                    <p className="text-sm text-white mb-1"><span className="text-gray-500 font-mono text-xs mr-2">EMAIL:</span> {order.customer_email || 'N/A'}</p>
                                    <p className="text-sm text-white"><span className="text-gray-500 font-mono text-xs mr-2">PHONE:</span> {order.customer_phone || 'N/A'}</p>
                                  </div>
                                  <div>
                                  ...
                                  <div>
                                    ...
                                    <div className="space-y-1 text-sm font-mono text-gray-400">
                                      ...
                                      <div className="flex justify-between border-t border-dashed border-gray-700 pt-2 mt-2 text-white font-bold text-lg">
                                     ...
                                     {order.items && order.items.map((item, idx) => (
                                        <div key={idx} className="bg-gray-900/50 p-3 rounded flex justify-between items-center border border-gray-800">
                                          <div className="flex gap-3 items-center">
                                            {item.image && <img src={item.image} alt="product" className="w-10 h-10 object-contain bg-gray-900 rounded border border-gray-700" />}
                                            <div>
                                          ...
                                          <span className="text-sm font-bold text-ion">${(item.price * item.quantity).toFixed(2)}</span>
                                        </div>
                                     ))}
                                   </div>
                                </div>
        ...
        {activeTab === 'newsletter' && (
          <div className="animate-fadeIn">
            <h1 className="text-3xl font-bold uppercase tracking-widest mb-8 border-b border-gray-800 pb-4">Newsletter Command Center</h1>
            
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              <div>
                <h2 className="text-xl font-bold uppercase tracking-widest mb-6 text-gray-400 border-b border-gray-800 pb-2">Welcome Email Template</h2>
                <form onSubmit={handleNewsletterSubmit} className="bg-[#0f1115] border border-gray-800 p-8 rounded-xl space-y-6">
                  
                  <div>
                    <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Subject Line</label>
                    <input required type="text" value={newsletterData.subject} onChange={e => setNewsletterData({...newsletterData, subject: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-electric transition-colors text-sm" />
                  </div>
                  
                  <div>
                    <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Email Headline</label>
                    <input required type="text" value={newsletterData.heading} onChange={e => setNewsletterData({...newsletterData, heading: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-electric transition-colors text-sm font-bold uppercase tracking-wider" />
                  </div>
                  
                  <div>
                    <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Main Message</label>
                    <textarea required rows="4" value={newsletterData.message} onChange={e => setNewsletterData({...newsletterData, message: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-electric transition-colors resize-none text-sm"></textarea>
                  </div>

                  <div>
                    <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Promo Code (Included in Box)</label>
                    <input required type="text" value={newsletterData.promo_code} onChange={e => setNewsletterData({...newsletterData, promo_code: e.target.value.toUpperCase()})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-ion focus:outline-none focus:border-ion transition-colors font-mono font-bold tracking-widest" />
                  </div>

                  <button type="submit" disabled={isSubmitting} className={`w-full font-bold uppercase tracking-widest py-4 rounded transition-all duration-300 mt-4 ${isSubmitting ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-red-600 text-white hover:bg-red-500'}`}>
              ...
              <div>
                <div className="flex justify-between items-end mb-6 border-b border-gray-800 pb-2">
                  <h2 className="text-xl font-bold uppercase tracking-widest text-gray-400">Subscriber Grid</h2>
                  <span className="text-electric text-xs font-bold uppercase tracking-widest bg-electric/10 px-3 py-1 rounded">
                ...
                <div className="bg-[#0f1115] border border-gray-800 rounded-xl overflow-hidden">
                  <div className="max-h-[600px] overflow-y-auto custom-scrollbar">
                    {subscribersList.length === 0 ? (
                      <div className="p-8 text-center text-gray-500 text-sm font-mono">No users on the grid yet.</div>
                    ) : (
                      subscribersList.map((sub, idx) => (
                        <div key={sub.id} className={`p-4 flex flex-col gap-1 ${idx !== subscribersList.length - 1 ? 'border-b border-gray-800/50' : ''}`}>
                          <div className="flex justify-between items-center">
                            <span className="font-mono text-sm text-gray-300">{sub.email}</span>
                          </div>
                          <span className="text-[10px] text-gray-600 font-mono">JOINED: {new Date(sub.created_at).toLocaleString()}</span>
                        </div>
                      ))
                    )}
                  </div>
        ...
        {activeTab === 'pages' && (
          <div className="animate-fadeIn">
            ...
            <form onSubmit={handleCmsSubmit} className="space-y-8">
              ...
              {activePage === 'shop' && (
                <div className="bg-[#0f1115] border border-gray-800 p-8 rounded-xl space-y-6">
                  <h3 className="text-ion font-bold uppercase tracking-widest border-b border-gray-800 pb-2 mb-4">Shop Configuration</h3>
                  <div>
                    <label className="block text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-2">Featured Flagship Product Override</label>
                    <select value={pageContent.flagship_id} onChange={e => setPageContent({...pageContent, flagship_id: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-ion transition-colors cursor-pointer text-sm">
                      <option value="">-- Auto-select Newest Gear --</option>
                      {productsList.map(product => (<option key={product.id} value={product.id}>[{product.category}] {product.name}</option>))}
                    </select>
                    <p className="text-gray-600 text-[10px] mt-2 font-mono uppercase tracking-widest">Selecting a product forces it to be the giant featured item on the shop page.</p>
                  </div>
                </div>
              )}
              ...
              {activePage === 'offers' && (
                <div className="bg-[#0f1115] border border-gray-800 p-8 rounded-xl space-y-6">
                  <h3 className="text-ion font-bold uppercase tracking-widest border-b border-gray-800 pb-2 mb-4">Offers Configuration</h3>
                  <div>
                    <label className="block text-gray-400 text-xs font-bold uppercase tracking-widest mb-2">Terms & Conditions Disclaimer</label>
                    <textarea rows="3" value={pageContent.offers_terms} onChange={e => setPageContent({...pageContent, offers_terms: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-white resize-none"></textarea>
                  </div>
                </div>
              )}
              ...
              {activePage === 'global' && (
                <div className="bg-[#0f1115] border border-gray-800 p-8 rounded-xl space-y-6">
                  <h3 className="text-ion font-bold uppercase tracking-widest border-b border-gray-800 pb-2 mb-4">Global Support Settings</h3>
        ...
        {activeTab === 'discounts' && (
          <div className="animate-fadeIn">
            <h1 className="text-3xl font-bold uppercase tracking-widest mb-8 border-b border-gray-800 pb-4">Manage Discounts</h1>
            
            <form onSubmit={handleDiscountSubmit} className="bg-[#0f1115] border border-gray-800 p-8 rounded-xl space-y-6 mb-12">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-gray-400 text-xs font-bold uppercase tracking-widest mb-2">Select Product</label>
                  <select 
                    required 
                    value={discountForm.product_id} 
                    onChange={e => setDiscountForm({...discountForm, product_id: e.target.value})} 
                    className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-ion transition-colors cursor-pointer text-sm"
                  >
                ...
                <div>
                  <label className="block text-gray-400 text-xs font-bold uppercase tracking-widest mb-2">Discount (%)</label>
                  <input required type="number" min="1" max="100" value={discountForm.discount_percent} onChange={e => setDiscountForm({...discountForm, discount_percent: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-ion transition-colors" />
                </div>
                <div>
                  <label className="block text-gray-400 text-xs font-bold uppercase tracking-widest mb-2">Expiration Date & Time (Optional)</label>
                  <input type="datetime-local" value={discountForm.expires_at} onChange={e => setDiscountForm({...discountForm, expires_at: e.target.value})} className="w-full bg-gray-900 border border-gray-700 rounded px-4 py-3 text-white focus:outline-none focus:border-ion transition-colors [color-scheme:dark]" />
                </div>
              </div>
              <button type="submit" disabled={isSubmitting} className={`w-full font-bold uppercase tracking-widest py-4 rounded transition-all duration-300 mt-4 ${isSubmitting ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-ion text-black hover:bg-white'}`}>
                Apply Discount
              </button>
            </form>

            <h2 className="text-xl font-bold uppercase tracking-widest mb-6 border-b border-gray-800 pb-2 text-gray-400">Active Product Discounts</h2>
            {discountedProducts.length === 0 ? (
              <div className="bg-[#0f1115] border border-gray-800 p-8 rounded-xl text-center text-gray-500 text-sm font-mono">
                No active individual product discounts.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {discountedProducts.map(p => (
                  <div key={p.id} className="bg-gray-900 border border-ion/30 p-4 rounded flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{p.category}</span>
                      <p className="font-bold text-sm truncate w-48 text-white">{p.name}</p>
                      <div className="flex gap-2 mt-1">
                        <span className="text-ion text-xs font-bold">{p.discount_percent}% OFF</span>
                        <span className="text-gray-500 text-xs line-through">${p.price.toFixed(2)}</span>
                        <span className="text-white text-xs">${(p.price - (p.price * (p.discount_percent / 100))).toFixed(2)}</span>
                      </div>
                    </div>
                    <button onClick={() => handleRemoveDiscount(p.id)} className="text-xs bg-red-900/20 text-red-500 border border-red-900/50 hover:bg-red-500 hover:text-white px-3 py-1 rounded transition-colors font-bold uppercase tracking-widest">
                      Remove
                    </button>
                  </div>
                ))}
